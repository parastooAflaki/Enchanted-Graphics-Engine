...
namespace ege {
    ...
    EgeModel::EgeModel(EgeDevice& device, const std::vector<Vertex>& vertices) : egeDevice{ device } {
    ...
    void EgeModel::createVertexBuffers(const std::vector<Vertex>& vertices) {
        ...
        egeDevice.createBuffer(
            ...
            vertexBufferMemory);
        void* data;
        vkMapMemory(egeDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
        memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
        vkUnmapMemory(egeDevice.device(), vertexBufferMemory);
    }
